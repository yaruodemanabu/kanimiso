...
pub struct LeastSquaresSolution {
    /// SVD minimum-norm coefficient vector.
    pub coefficients: Vector,
    /// Decomposition used by the solve and its diagnostics.
    pub decomposition: ThinSvd,
    /// Numerical rank under the supplied policy.
    pub rank: usize,
}

/// Result of one Cholesky factorization with one or more right-hand sides.
#[derive(Clone, Debug)]
pub struct SpdSolveSolution {
    /// One solution column per right-hand side.
    pub solution: Matrix,
    /// Log determinant from the same Cholesky factorization.
...
pub fn thin_svd(report: &mut Report, x: &Matrix, policy: &Policy) -> Option<ThinSvd> {
    match x.inner().thin_svd() {
        Ok(svd) => {
            ...
            // Reuse the factorization's values: a second SVD is both redundant
            // and can mismatch U/V near clustered singular values.
            let diagonal = svd.S().column_vector();
            let s = (0..diagonal.nrows()).map(|i| diagonal[i]).collect();
            Some(ThinSvd {
                u,
                singular_values: s,
                v,
...
pub fn least_squares_with_diagnostics(
    report: &mut Report,
    x: &Matrix,
    y: &Vector,
    policy: &Policy,
...
pub fn chol_solve_matrix_with_context(
    report: &mut Report,
    a: &Mat<f64>,
    b: &Matrix,
    policy: &Policy,
...
pub fn matrix_symmetrized(matrix: &Matrix) -> Matrix {
