...
use signlred::{
    ...
    FiniteScan, Issue, IssueCode, Meaninglessness, Policy, Report, SliceStats,
};
...
pub fn inspect_xy(report: &mut Report, x: &Matrix, y: Option<&Vector>, policy: &Policy) {
    inspect_xy_internal(report, x, y, policy, true, false);
}

/// Inspect `X, y` while treating NaN features as supported missing values.
///
/// Infinities remain errors, and an entirely missing column is still reported.
pub fn inspect_xy_allow_missing_features(
    report: &mut Report,
    x: &Matrix,
    y: Option<&Vector>,
    policy: &Policy,
) {
    inspect_xy_internal(report, x, y, policy, true, true);
}
...
pub fn inspect_xy_allow_constant_target(
    report: &mut Report,
    x: &Matrix,
    y: &Vector,
    policy: &Policy,
) {
    inspect_xy_internal(report, x, Some(y), policy, false, false);
}
...
fn inspect_xy_internal(
    ...
    let feature_scan = scan_finite(&buf);
    let feature_scan = if allow_missing_features {
        FiniteScan {
            nans: 0,
            pos_infs: feature_scan.pos_infs,
            neg_infs: feature_scan.neg_infs,
            first_bad: buf.iter().position(|value| value.is_infinite()),
        }
    } else {
        feature_scan
    };
    if let Some(issue) = feature_scan.to_issue("X") {
