...
/// positive-infinity maximum returns positive infinity. NaN always propagates.
pub fn logsumexp(xs: &[f64]) -> f64 {
    if xs.iter().any(|value| value.is_nan()) {
        return f64::NAN;
    }
...
mod tests {
    ...
    #[test]
    fn logsumexp_never_hides_nan_behind_an_infinite_maximum() {
        for value in [f64::NEG_INFINITY, 0.0, f64::INFINITY] {
            assert!(logsumexp(&[f64::NAN, value]).is_nan());
            assert!(logsumexp(&[value, f64::NAN]).is_nan());
        }
    }
    ...
    fn scipy_golden_replay() {
        let raw = include_str!("../special_functions.json");
