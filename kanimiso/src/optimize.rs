...
impl NelderMead {
    ...
    pub(crate) fn minimize_nested<F>(
        &self,
        simplex: &[Vector],
        objective: F,
        session: &Session,
...
pub(crate) fn encode_open_interval(value: f64, lower: f64, upper: f64) -> Option<f64> {
...
pub(crate) fn decode_open_interval(value: f64, lower: f64, upper: f64) -> Option<f64> {
    let width = upper - lower;
...
mod tests {
    ...
    fn invalid_configuration_simplex_and_objective_fail_explicitly() {
        ...
        let mut invalid = NelderMead::default();
        invalid.contraction = 0.75;
        let failure = invalid
    ...
    fn parameter_resolution_without_objective_agreement_is_not_convergence() {
        ...
        policy.optimizer_parameter_tol = 1.0e6;
        policy.optimizer_objective_tol = f64::MIN_POSITIVE;
        let config = NelderMead {
            policy,
            ..NelderMead::default()
        };
