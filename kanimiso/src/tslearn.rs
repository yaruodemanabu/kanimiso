...
fn minkowski42_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=42\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski42_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski42_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski42_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski42_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski42_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski42_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}

fn minkowski43_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=43\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski43_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski43_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski43_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski43_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski43_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski43_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}

fn minkowski44_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=44\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski44_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski44_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski44_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski44_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski44_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski44_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}

fn minkowski45_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=45\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski45_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski45_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski45_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski45_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski45_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski45_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}

fn minkowski46_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=46\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski46_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski46_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski46_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski46_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski46_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski46_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}

fn minkowski47_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=47\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski47_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski47_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski47_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski47_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski47_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski47_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}

fn minkowski48_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=48\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski48_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski48_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski48_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski48_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski48_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski48_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski49_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=49\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski49_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski49_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski49_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski49_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski49_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski49_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}
...
fn minkowski50_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=50\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski50_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski50_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski50_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski50_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski50_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski50_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}
...
fn minkowski51_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=51\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski51_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski51_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski51_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski51_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski51_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski51_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}
...
fn minkowski52_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=52\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski52_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski52_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski52_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski52_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski52_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski52_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}
...
fn minkowski53_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=53\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski53_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski53_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski53_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski53_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski53_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski53_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}
...
fn minkowski54_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=54\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski54_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski54_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski54_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski54_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski54_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski54_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski55_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=55\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski55_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski55_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski55_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski55_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski55_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski55_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski56_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=56\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski56_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski56_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski56_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski56_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski56_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski56_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski57_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=57\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski57_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski57_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski57_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski57_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski57_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski57_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski58_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=58\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski58_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski58_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski58_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski58_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski58_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski58_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski59_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=59\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski59_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski59_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski59_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski59_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski59_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski59_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski60_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=60\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski60_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski60_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski60_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski60_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski60_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski60_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski61_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=61\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski61_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski61_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski61_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski61_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski61_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski61_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski62_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=62\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski62_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski62_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski62_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski62_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski62_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski62_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski63_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=63\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski63_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski63_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski63_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski63_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski63_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski63_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski64_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=64\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski64_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski64_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski64_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski64_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski64_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski64_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski65_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=65\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski65_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski65_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski65_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski65_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski65_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski65_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski66_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=66\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski66_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski66_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski66_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski66_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski66_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski66_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski67_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=67\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski67_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski67_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski67_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski67_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski67_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski67_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski68_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=68\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski68_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski68_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski68_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski68_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski68_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski68_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski69_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=69\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski69_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski69_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski69_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski69_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski69_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski69_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski70_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=70\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski70_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski70_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski70_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski70_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski70_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski70_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski71_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=71\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski71_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski71_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski71_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski71_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski71_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski71_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski72_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=72\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski72_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski72_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski72_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski72_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski72_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski72_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski73_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=73\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski73_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski73_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski73_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski73_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski73_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski73_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski74_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=74\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski74_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski74_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski74_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski74_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski74_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski74_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski75_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=75\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski75_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski75_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski75_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski75_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski75_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski75_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski76_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=76\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski76_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski76_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski76_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski76_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski76_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski76_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}


fn minkowski77_l1_distance_raw(a: &[f64], b: &[f64]) -> f64 {
    let n = a.len().min(b.len());
    if n == 0 {
        return f64::NAN;
    }
...
/// Minkowski \(p=77\) distance after \(\ell_1\) normalisation.
///
...
pub fn minkowski77_l1_distance(a: &Vector, b: &Vector, session: &Session) -> Result<Qualified<f64>> {
    ...
    if let Some(issue) = signlred::scan_finite(b.as_slice()).to_issue("minkowski77_l1_distance.b") {
        ctx.push(issue);
    }
    if a.is_empty() || b.is_empty() {
        ctx.push(
            Issue::builder(IssueCode::EmptyMatrix)
                .message("minkowski77_l1_distance on an empty series")
                .build(),
        );
        return ctx.finish(f64::NAN);
    }
    ctx.finish(minkowski77_l1_distance_raw(a.as_slice(), b.as_slice()))
}
...
pub fn cdist_minkowski77_l1(a: &Matrix, b: &Matrix, session: &Session) -> Result<Qualified<Matrix>> {
    ...
    let out = Matrix::from_fn(a.nrows(), b.nrows(), |i, j| {
        let ai = a.row(i);
        let bj = b.row(j);
        minkowski77_l1_distance_raw(ai.as_slice(), bj.as_slice())
    });
    ctx.finish(out)
}
...
mod tests {
    ...
    fn dtw_identical_is_zero() {
        ...
        let m42 = minkowski42_l1_distance(&a, &a, &Session::new("ts", "m42")).unwrap().value;
        assert!(m42.abs() < 1e-12, "minkowski42_l1_distance={m42}");
        let m43 = minkowski43_l1_distance(&a, &a, &Session::new("ts", "m43")).unwrap().value;
        assert!(m43.abs() < 1e-12, "minkowski43_l1_distance={m43}");
        let m44 = minkowski44_l1_distance(&a, &a, &Session::new("ts", "m44")).unwrap().value;
        assert!(m44.abs() < 1e-12, "minkowski44_l1_distance={m44}");
        let m45 = minkowski45_l1_distance(&a, &a, &Session::new("ts", "m45")).unwrap().value;
        assert!(m45.abs() < 1e-12, "minkowski45_l1_distance={m45}");
        let m46 = minkowski46_l1_distance(&a, &a, &Session::new("ts", "m46")).unwrap().value;
        assert!(m46.abs() < 1e-12, "minkowski46_l1_distance={m46}");
        let m47 = minkowski47_l1_distance(&a, &a, &Session::new("ts", "m47")).unwrap().value;
        assert!(m47.abs() < 1e-12, "minkowski47_l1_distance={m47}");
        let m48 = minkowski48_l1_distance(&a, &a, &Session::new("ts", "m48")).unwrap().value;
        assert!(m48.abs() < 1e-12, "minkowski48_l1_distance={m48}");
        let m49 = minkowski49_l1_distance(&a, &a, &Session::new("ts", "m49")).unwrap().value;
        assert!(m49.abs() < 1e-12, "minkowski49_l1_distance={m49}");
        let m50 = minkowski50_l1_distance(&a, &a, &Session::new("ts", "m50")).unwrap().value;
        assert!(m50.abs() < 1e-12, "minkowski50_l1_distance={m50}");
        let m51 = minkowski51_l1_distance(&a, &a, &Session::new("ts", "m51")).unwrap().value;
        assert!(m51.abs() < 1e-12, "minkowski51_l1_distance={m51}");
        let m52 = minkowski52_l1_distance(&a, &a, &Session::new("ts", "m52")).unwrap().value;
        assert!(m52.abs() < 1e-12, "minkowski52_l1_distance={m52}");
        let m53 = minkowski53_l1_distance(&a, &a, &Session::new("ts", "m53")).unwrap().value;
        assert!(m53.abs() < 1e-12, "minkowski53_l1_distance={m53}");
        let m54 = minkowski54_l1_distance(&a, &a, &Session::new("ts", "m54")).unwrap().value;
        assert!(m54.abs() < 1e-12, "minkowski54_l1_distance={m54}");
        let m55 = minkowski55_l1_distance(&a, &a, &Session::new("ts", "m55")).unwrap().value;
        assert!(m55.abs() < 1e-12, "minkowski55_l1_distance={m55}");
        let m56 = minkowski56_l1_distance(&a, &a, &Session::new("ts", "m56")).unwrap().value;
        assert!(m56.abs() < 1e-12, "minkowski56_l1_distance={m56}");
        let m57 = minkowski57_l1_distance(&a, &a, &Session::new("ts", "m57")).unwrap().value;
        assert!(m57.abs() < 1e-12, "minkowski57_l1_distance={m57}");
        let m58 = minkowski58_l1_distance(&a, &a, &Session::new("ts", "m58")).unwrap().value;
        assert!(m58.abs() < 1e-12, "minkowski58_l1_distance={m58}");
        let m59 = minkowski59_l1_distance(&a, &a, &Session::new("ts", "m59")).unwrap().value;
        assert!(m59.abs() < 1e-12, "minkowski59_l1_distance={m59}");
        let m60 = minkowski60_l1_distance(&a, &a, &Session::new("ts", "m60")).unwrap().value;
        assert!(m60.abs() < 1e-12, "minkowski60_l1_distance={m60}");
        let mk61 = minkowski61_l1_distance(&a, &a, &Session::new("ts", "mk61")).unwrap().value;
        assert!(mk61.abs() < 1e-12, "minkowski61_l1_distance={mk61}");
        let m62 = minkowski62_l1_distance(&a, &a, &Session::new("ts", "m62")).unwrap().value;
        assert!(m62.abs() < 1e-12, "minkowski62_l1_distance={m62}");
        let m63 = minkowski63_l1_distance(&a, &a, &Session::new("ts", "m63")).unwrap().value;
        assert!(m63.abs() < 1e-12, "minkowski63_l1_distance={m63}");
        let m64 = minkowski64_l1_distance(&a, &a, &Session::new("ts", "m64")).unwrap().value;
        assert!(m64.abs() < 1e-12, "minkowski64_l1_distance={m64}");
        let m65 = minkowski65_l1_distance(&a, &a, &Session::new("ts", "m65")).unwrap().value;
        assert!(m65.abs() < 1e-12, "minkowski65_l1_distance={m65}");
        let m66 = minkowski66_l1_distance(&a, &a, &Session::new("ts", "m66")).unwrap().value;
        assert!(m66.abs() < 1e-12, "minkowski66_l1_distance={m66}");
        let m67 = minkowski67_l1_distance(&a, &a, &Session::new("ts", "m67")).unwrap().value;
        assert!(m67.abs() < 1e-12, "minkowski67_l1_distance={m67}");
        let m68 = minkowski68_l1_distance(&a, &a, &Session::new("ts", "m68")).unwrap().value;
        assert!(m68.abs() < 1e-12, "minkowski68_l1_distance={m68}");
        let m69 = minkowski69_l1_distance(&a, &a, &Session::new("ts", "m69")).unwrap().value;
        assert!(m69.abs() < 1e-12, "minkowski69_l1_distance={m69}");
        let m70 = minkowski70_l1_distance(&a, &a, &Session::new("ts", "m70")).unwrap().value;
        assert!(m70.abs() < 1e-12, "minkowski70_l1_distance={m70}");
        let mk71 = minkowski71_l1_distance(&a, &a, &Session::new("ts", "mk71")).unwrap().value;
        assert!(mk71.abs() < 1e-12, "minkowski71_l1_distance={mk71}");
        let m72 = minkowski72_l1_distance(&a, &a, &Session::new("ts", "m72")).unwrap().value;
        assert!(m72.abs() < 1e-12, "minkowski72_l1_distance={m72}");
        let m73 = minkowski73_l1_distance(&a, &a, &Session::new("ts", "m73")).unwrap().value;
        assert!(m73.abs() < 1e-12, "minkowski73_l1_distance={m73}");
        let m74 = minkowski74_l1_distance(&a, &a, &Session::new("ts", "m74")).unwrap().value;
        assert!(m74.abs() < 1e-12, "minkowski74_l1_distance={m74}");
        let m75 = minkowski75_l1_distance(&a, &a, &Session::new("ts", "m75")).unwrap().value;
        assert!(m75.abs() < 1e-12, "minkowski75_l1_distance={m75}");
        let m76 = minkowski76_l1_distance(&a, &a, &Session::new("ts", "m76")).unwrap().value;
        assert!(m76.abs() < 1e-12, "minkowski76_l1_distance={m76}");
        let m77 = minkowski77_l1_distance(&a, &a, &Session::new("ts", "m77")).unwrap().value;
        assert!(m77.abs() < 1e-12, "minkowski77_l1_distance={m77}");
    }
    ...
    fn dtw_kmeans_and_svm() {
        ...
        let cd42 = cdist_minkowski42_l1(&x, &x, &Session::new("ts", "cd42"))
            .unwrap()
            .value;
        assert_eq!(cd42.shape(), (8, 8));
        assert!(cd42.get(0, 0).abs() < 1e-12);
        let cd43 = cdist_minkowski43_l1(&x, &x, &Session::new("ts", "cd43"))
            .unwrap()
            .value;
        assert_eq!(cd43.shape(), (8, 8));
        assert!(cd43.get(0, 0).abs() < 1e-12);
        let cd44 = cdist_minkowski44_l1(&x, &x, &Session::new("ts", "cd44"))
            .unwrap()
            .value;
        assert_eq!(cd44.shape(), (8, 8));
        assert!(cd44.get(0, 0).abs() < 1e-12);
        let cd45 = cdist_minkowski45_l1(&x, &x, &Session::new("ts", "cd45"))
            .unwrap()
            .value;
        assert_eq!(cd45.shape(), (8, 8));
        assert!(cd45.get(0, 0).abs() < 1e-12);
        let cd46 = cdist_minkowski46_l1(&x, &x, &Session::new("ts", "cd46"))
            .unwrap()
            .value;
        assert_eq!(cd46.shape(), (8, 8));
        assert!(cd46.get(0, 0).abs() < 1e-12);
        let cd47 = cdist_minkowski47_l1(&x, &x, &Session::new("ts", "cd47"))
            .unwrap()
            .value;
        assert_eq!(cd47.shape(), (8, 8));
        assert!(cd47.get(0, 0).abs() < 1e-12);
        let cd48 = cdist_minkowski48_l1(&x, &x, &Session::new("ts", "cd48"))
            .unwrap()
            .value;
        assert_eq!(cd48.shape(), (8, 8));
        assert!(cd48.get(0, 0).abs() < 1e-12);
        let cd49 = cdist_minkowski49_l1(&x, &x, &Session::new("ts", "cd49"))
            .unwrap()
            .value;
        assert_eq!(cd49.shape(), (8, 8));
        assert!(cd49.get(0, 0).abs() < 1e-12);
        let cd50 = cdist_minkowski50_l1(&x, &x, &Session::new("ts", "cd50"))
            .unwrap()
            .value;
        assert_eq!(cd50.shape(), (8, 8));
        assert!(cd50.get(0, 0).abs() < 1e-12);
        let cd51 = cdist_minkowski51_l1(&x, &x, &Session::new("ts", "cd51"))
            .unwrap()
            .value;
        assert_eq!(cd51.shape(), (8, 8));
        assert!(cd51.get(0, 0).abs() < 1e-12);
        let cd52 = cdist_minkowski52_l1(&x, &x, &Session::new("ts", "cd52"))
            .unwrap()
            .value;
        assert_eq!(cd52.shape(), (8, 8));
        assert!(cd52.get(0, 0).abs() < 1e-12);
        let cd53 = cdist_minkowski53_l1(&x, &x, &Session::new("ts", "cd53"))
            .unwrap()
            .value;
        assert_eq!(cd53.shape(), (8, 8));
        assert!(cd53.get(0, 0).abs() < 1e-12);
        let cd54 = cdist_minkowski54_l1(&x, &x, &Session::new("ts", "cd54"))
            .unwrap()
            .value;
        assert_eq!(cd54.shape(), (8, 8));
        assert!(cd54.get(0, 0).abs() < 1e-12);
        let cd55 = cdist_minkowski55_l1(&x, &x, &Session::new("ts", "cd55"))
            .unwrap()
            .value;
        assert_eq!(cd55.shape(), (8, 8));
        assert!(cd55.get(0, 0).abs() < 1e-12);
        let cd56 = cdist_minkowski56_l1(&x, &x, &Session::new("ts", "cd56"))
            .unwrap()
            .value;
        assert_eq!(cd56.shape(), (8, 8));
        assert!(cd56.get(0, 0).abs() < 1e-12);
        let cd57 = cdist_minkowski57_l1(&x, &x, &Session::new("ts", "cd57"))
            .unwrap()
            .value;
        assert_eq!(cd57.shape(), (8, 8));
        assert!(cd57.get(0, 0).abs() < 1e-12);
        let cd58 = cdist_minkowski58_l1(&x, &x, &Session::new("ts", "cd58"))
            .unwrap()
            .value;
        assert_eq!(cd58.shape(), (8, 8));
        assert!(cd58.get(0, 0).abs() < 1e-12);
        let cd59 = cdist_minkowski59_l1(&x, &x, &Session::new("ts", "cd59"))
            .unwrap()
            .value;
        assert_eq!(cd59.shape(), (8, 8));
        assert!(cd59.get(0, 0).abs() < 1e-12);
        let cd60 = cdist_minkowski60_l1(&x, &x, &Session::new("ts", "cd60"))
            .unwrap()
            .value;
        assert_eq!(cd60.shape(), (8, 8));
        assert!(cd60.get(0, 0).abs() < 1e-12);
        let cdk61 = cdist_minkowski61_l1(&x, &x, &Session::new("ts", "cdk61"))
            .unwrap()
            .value;
        assert_eq!(cdk61.shape(), (8, 8));
        assert!(cdk61.get(0, 0).abs() < 1e-12);
        let cd62 = cdist_minkowski62_l1(&x, &x, &Session::new("ts", "cd62"))
            .unwrap()
            .value;
        assert_eq!(cd62.shape(), (8, 8));
        assert!(cd62.get(0, 0).abs() < 1e-12);
        let cd63 = cdist_minkowski63_l1(&x, &x, &Session::new("ts", "cd63"))
            .unwrap()
            .value;
        assert_eq!(cd63.shape(), (8, 8));
        assert!(cd63.get(0, 0).abs() < 1e-12);
        let cd64 = cdist_minkowski64_l1(&x, &x, &Session::new("ts", "cd64"))
            .unwrap()
            .value;
        assert_eq!(cd64.shape(), (8, 8));
        assert!(cd64.get(0, 0).abs() < 1e-12);
        let cd65 = cdist_minkowski65_l1(&x, &x, &Session::new("ts", "cd65"))
            .unwrap()
            .value;
        assert_eq!(cd65.shape(), (8, 8));
        assert!(cd65.get(0, 0).abs() < 1e-12);
        let cd66 = cdist_minkowski66_l1(&x, &x, &Session::new("ts", "cd66"))
            .unwrap()
            .value;
        assert_eq!(cd66.shape(), (8, 8));
        assert!(cd66.get(0, 0).abs() < 1e-12);
        let cd67 = cdist_minkowski67_l1(&x, &x, &Session::new("ts", "cd67"))
            .unwrap()
            .value;
        assert_eq!(cd67.shape(), (8, 8));
        assert!(cd67.get(0, 0).abs() < 1e-12);
        let cd68 = cdist_minkowski68_l1(&x, &x, &Session::new("ts", "cd68"))
            .unwrap()
            .value;
        assert_eq!(cd68.shape(), (8, 8));
        assert!(cd68.get(0, 0).abs() < 1e-12);
        let cd69 = cdist_minkowski69_l1(&x, &x, &Session::new("ts", "cd69"))
            .unwrap()
            .value;
        assert_eq!(cd69.shape(), (8, 8));
        assert!(cd69.get(0, 0).abs() < 1e-12);
        let cd70 = cdist_minkowski70_l1(&x, &x, &Session::new("ts", "cd70"))
            .unwrap()
            .value;
        assert_eq!(cd70.shape(), (8, 8));
        assert!(cd70.get(0, 0).abs() < 1e-12);
        let cdk71 = cdist_minkowski71_l1(&x, &x, &Session::new("ts", "cdk71"))
            .unwrap()
            .value;
        assert_eq!(cdk71.shape(), (8, 8));
        assert!(cdk71.get(0, 0).abs() < 1e-12);
        let cd72 = cdist_minkowski72_l1(&x, &x, &Session::new("ts", "cd72"))
            .unwrap()
            .value;
        assert_eq!(cd72.shape(), (8, 8));
        assert!(cd72.get(0, 0).abs() < 1e-12);
        let cd73 = cdist_minkowski73_l1(&x, &x, &Session::new("ts", "cd73"))
            .unwrap()
            .value;
        assert_eq!(cd73.shape(), (8, 8));
        assert!(cd73.get(0, 0).abs() < 1e-12);
        let cd74 = cdist_minkowski74_l1(&x, &x, &Session::new("ts", "cd74"))
            .unwrap()
            .value;
        assert_eq!(cd74.shape(), (8, 8));
        assert!(cd74.get(0, 0).abs() < 1e-12);
        let cd75 = cdist_minkowski75_l1(&x, &x, &Session::new("ts", "cd75"))
            .unwrap()
            .value;
        assert_eq!(cd75.shape(), (8, 8));
        assert!(cd75.get(0, 0).abs() < 1e-12);
        let cd76 = cdist_minkowski76_l1(&x, &x, &Session::new("ts", "cd76"))
            .unwrap()
            .value;
        assert_eq!(cd76.shape(), (8, 8));
        assert!(cd76.get(0, 0).abs() < 1e-12);
        let cd77 = cdist_minkowski77_l1(&x, &x, &Session::new("ts", "cd77"))
            .unwrap()
            .value;
        assert_eq!(cd77.shape(), (8, 8));
        assert!(cd77.get(0, 0).abs() < 1e-12);
